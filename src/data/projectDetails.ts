...
model = get_peft_model(model, lora_config)`,
  ...
  'lpdr': {
    introduction:
      'A Comparative Study on Detection and Recognition of Nonuniform License Plates — peer-reviewed research published in Big Data and Cognitive Computing (2024, IF 3.7). Co-authored with Mehak Arshid and Zahid Mahmood at COMSATS University Islamabad, Abbottabad. The study benchmarks four deep learning pipelines on a custom Pakistani traffic dataset captured in unconstrained outdoor environments with province-specific plate styles, fancy plates, embossed characters, and abrupt illumination changes.',
    problemStatement:
      'Pakistan lacks federal LP standardization: plates vary in color, font, layout, and mounting. Existing ALPR systems trained on uniform templates fail on multilingual, nonstandard, and low-resolution Pakistani plates captured from highways, malls, and parking lots.',
    objectives: [
      'Compare Faster-RCNN and end-to-end (E2E) methods for license plate detection',
      'Compare DNN and CA-CenterNet architectures for multi-style LP recognition',
      'Measure computational complexity from 1550×900 down to 600×400 pixel resolutions',
      'Identify failure modes (broken, occluded, embossed, and faded plates) in real Pakistani traffic',
    ],
    methodology: [
      'Collected 16,521 annotated objects (cars, motorcycles, pedestrians) with visible plates; 80:20 train/test split',
      'Labeled with labelImg; low-density (≤2 objects/image) and high-density scenes from highways and parking lots',
      'Detection pool: Faster-RCNN + LPLM (HSV/morphology) vs E2E single-forward-pass detector',
      'Recognition pool: DNN character recognition vs CA-CenterNet (ResNet50, 512×512) with segmentation-free head',
      'Python 3.11 on HP Omen Core i7 (14th gen), 32 GB RAM; SGDM optimizer, lr 10⁻⁴, 50 epochs, batch 32',
    ],
    results:
      'Detection: Faster-RCNN 98.35%, E2E 98.48% (mean 98.41%). Recognition: DNN 98.90%, CA-CenterNet 98.96% (pool mean 98.93%). Combined pipeline mean accuracy 98.67%. CA-CenterNet fastest at all resolutions (≈1.0 s at 600×400); E2E recommended for real-time detection, CA-CenterNet for real-time recognition. Mean performance: CA-CenterNet 97%, E2E 96%, DNN 95.83%, Faster-RCNN 91.91%.',
    gallery: [
      { title: 'Fig. 1 — Nonuniform LP Samples', src: ghMedia(p5, 'fig1-nonuniform-lp-samples.png') },
      { title: 'Fig. 2 — Dataset Samples', src: ghMedia(p5, 'fig2-dataset-samples.png') },
      { title: 'Fig. 3 — LP Detection (Faster-RCNN & E2E)', src: ghMedia(p5, 'fig3-lp-detection.png') },
      { title: 'Fig. 4 — LP Recognition (DNN & CA-CenterNet)', src: ghMedia(p5, 'fig4-lp-recognition.png') },
    ...
    references: [
      { label: 'MDPI Publication (DOI)', url: 'https://doi.org/10.3390/bdcc8110155' },
      { label: 'Article Page', url: 'https://www.mdpi.com/2504-2289/8/11/155' },
      { label: 'Dataset Portal', url: 'http://research.cuiatd.edu.pk/admin/login.aspx' },
    ],
  },
  'lungs-seg': {
    introduction:
...
flag_fraud = proba > optimal_threshold`,
  ...
  'toll-anpr': {
    ...
    gallery: [
      { title: 'LP Detection Results', src: ghMedia(p5, 'fig3-lp-detection.png') },
      { title: 'LP Recognition Results', src: ghMedia(p5, 'fig4-lp-recognition.png') },
      { title: 'Pakistani Plate Samples', src: ghMedia(p5, 'fig1-nonuniform-lp-samples.png') },
      { title: 'Challenging Plate Cases', src: ghMedia(p5, 'fig7-challenging-cases.png') },
    ],
  },
  'wheat-disease-yolo': {
    introduction:
      'YOLOv8-based wheat disease detector designed for early-stage field diagnosis and intervention support.',
    problemStatement:
      'Farmers often detect foliar disease late, resulting in lower yield and unnecessary pesticide overuse.',
    objectives: ['Detect major disease classes from RGB images', 'Support near real-time field screening'],
    methodology: ['Dataset curation and annotation', 'YOLOv8 fine-tuning with augmentation', 'Precision/recall evaluation'],
    results: 'Delivered practical field inference behavior and class-level confidence outputs for extension workflows.',
    gallery: [{ title: 'Wheat Disease Detection', src: ghMedia(p13, 'leaf-segmentation.png') }],
  },
  'maize-deficiency-cnn': {
    introduction:
      'CNN classifier for identifying nutrient deficiencies in maize leaves captured under controlled conditions.',
    problemStatement: 'Deficiency symptoms can be visually similar, creating classification errors in manual inspection.',
    objectives: ['Build multi-class deficiency classifier', 'Improve minority-class recall'],
    methodology: ['Image normalization and split strategy', 'CNN training with class weighting', 'Confusion-matrix analysis'],
    results: 'Established a repeatable baseline for maize deficiency analysis with consistent validation behavior.',
    gallery: [{ title: 'Deficiency Dataset', src: ghMedia(p14, 'fig5-deficiency-training-dataset.png') }],
  },
  'tomato-ripeness-cv': {
    introduction:
      'Computer vision system for grading tomato ripeness levels from RGB fruit images.',
    problemStatement: 'Manual grading is subjective and inconsistent across sorting operations.',
    objectives: ['Predict ripeness stage', 'Enable low-cost sorting decision support'],
    methodology: ['Color-space feature extraction', 'Classifier training and calibration', 'Threshold tuning per stage'],
    results: 'Produced stable ripeness-stage predictions suitable for small-scale processing units.',
    gallery: [{ title: 'Tomato Analysis Workflow', src: ghMedia(p7, 'app-1.png') }],
  },
  'greenhouse-anomaly-detection': {
    introduction:
      'Anomaly detection pipeline over greenhouse temperature, humidity, and control-state telemetry.',
    problemStatement: 'Undetected equipment drift and sensor faults can damage high-value greenhouse batches.',
    objectives: ['Detect unusual telemetry patterns', 'Trigger actionable alerts for operators'],
    methodology: ['Time-window feature engineering', 'Sequence modeling and thresholding', 'Operator feedback loop'],
    results: 'Reduced delayed-fault discovery and improved proactive maintenance response.',
    gallery: [{ title: 'Incubator Deployment', src: ghMedia(p13, 'incubator-deployment.png') }],
  },
  'drone-crop-counting': {
    introduction: 'Drone imagery pipeline for crop stand counting and emergence analytics.',
    problemStatement: 'Manual stand counting is labor-intensive and error-prone at plot scale.',
    objectives: ['Automate plot-level counting', 'Generate agronomy-friendly summaries'],
    methodology: ['Aerial image tiling', 'Detection-based counting', 'Plot aggregation and validation'],
    results: 'Provided fast stand-estimate outputs for early-season field assessment.',
    gallery: [{ title: 'Drone Deployment', src: ghMedia(p7, 'Deployment-1.png') }],
  },
  'weed-seg-unet': {
    introduction: 'U-Net baseline for pixel-level weed segmentation in mixed crop scenes.',
    problemStatement: 'Robotic and spray systems require mask-level precision for selective intervention.',
    objectives: ['Build robust segmentation baseline', 'Improve small-object weed masks'],
    methodology: ['U-Net training with augmentations', 'Dice-centric optimization', 'Overlay-based error analysis'],
    results: 'Improved weed mask continuity and boundary quality for downstream localization.',
    gallery: [{ title: 'Segmentation Comparison', src: ghMedia(p10, 'segmentation-comparison.png') }],
  },
  'orchard-fruit-detection': {
    introduction: 'Fruit detection and counting workflow for orchard productivity monitoring.',
    problemStatement: 'Occlusion and lighting variation complicate consistent fruit counts.',
    objectives: ['Detect fruits under canopy occlusion', 'Summarize per-tree counts'],
    methodology: ['Object detection training', 'Scene-level post-processing', 'Count consistency validation'],
    results: 'Delivered reliable counting trends useful for harvest planning.',
    gallery: [{ title: 'Orchard Monitoring', src: ghMedia(p13, 'cover.png') }],
  },
  'multispectral-plant-health': {
    introduction: 'Multispectral feature fusion for plant stress and vigor scoring.',
    problemStatement: 'RGB-only cues can miss early physiological stress signatures.',
    objectives: ['Estimate stress indicators', 'Compare fusion against single-source features'],
    methodology: ['Index extraction and scaling', 'Regressor training', 'Cross-plot validation'],
    results: 'Fusion features improved consistency of stress estimation across sample groups.',
    gallery: [{ title: 'Plant Segmentation Context', src: ghMedia(p7, 'FCN For Segmentation.png') }],
  },
  'seed-quality-classification': {
    introduction: 'Image-based seed quality classification for pre-processing and grading lines.',
    problemStatement: 'Manual seed screening introduces variability and throughput bottlenecks.',
    objectives: ['Classify quality grades', 'Support sorting decisions'],
    methodology: ['Seed image dataset preparation', 'CNN classifier training', 'Performance and class-error review'],
    results: 'Established a deployable seed-grade classifier prototype for teaching and demos.',
    gallery: [{ title: 'Seed Quality System', src: ghMedia(p6, 'cover.png') }],
  },
  'irrigation-demand-forecasting': {
    introduction: 'Forecasting model for estimating irrigation demand using telemetry and weather context.',
    problemStatement: 'Over- and under-irrigation reduce yield and waste water resources.',
    objectives: ['Predict near-term water demand', 'Generate scheduling recommendations'],
    methodology: ['Time-series feature engineering', 'Gradient boosting regressors', 'Forecast error tracking'],
    results: 'Produced practical demand forecasts for daily irrigation planning.',
    gallery: [{ title: 'Smart Incubator Context', src: ghMedia(p13, 'cisnr-lab-2.png') }],
  },
  'pest-detection-edge': {
    introduction: 'Low-power pest detection pipeline optimized for edge inference.',
    problemStatement: 'Pest outbreaks require quick detection where connectivity and compute are limited.',
    objectives: ['Detect key pest classes on-device', 'Maintain low-latency inference'],
    methodology: ['Model quantization and export', 'Edge benchmarking on Jetson', 'False-positive reduction loop'],
    results: 'Achieved robust on-device detection suitable for greenhouse pilots.',
    gallery: [{ title: 'Greenhouse Deployment', src: ghMedia(p13, 'incubator-deployment.png') }],
  },
  'leaf-instance-seg': {
    introduction: 'Instance segmentation pipeline for leaf-level masks in dense plant canopies.',
    problemStatement: 'Semantic masks alone cannot separate overlapping leaves for precise phenotyping.',
    objectives: ['Segment individual leaves', 'Improve area measurement fidelity'],
    methodology: ['Instance mask training', 'Augmentation for overlap scenarios', 'Mask-quality evaluation'],
    results: 'Generated stable per-leaf masks for downstream measurement tasks.',
    gallery: [{ title: 'Segmentation Pipeline', src: ghMedia(p14, 'fig-segmentation-pipeline.png') }],
  },
  'soil-moisture-fusion': {
    introduction: 'Fusion model combining vision cues and sensor streams to estimate soil moisture zones.',
    problemStatement: 'Single-modality moisture estimation is unstable under changing surface conditions.',
    objectives: ['Fuse visual and telemetry signals', 'Improve zone-level estimation consistency'],
    methodology: ['Feature fusion architecture', 'Calibration with ground probes', 'Zone-level mapping'],
    results: 'Improved moisture-zone discrimination compared with single-input baselines.',
    gallery: [{ title: 'Field Deployment', src: ghMedia(p7, 'Deployment-2.png') }],
  },
  'crop-yield-regression': {
    introduction: 'Regression-based framework for crop yield estimation from image and climate features.',
    problemStatement: 'Yield planning often relies on late-stage manual estimation with limited granularity.',
    objectives: ['Estimate yield earlier in season', 'Analyze key predictive factors'],
    methodology: ['Feature engineering and ablation', 'Gradient-boosting regressors', 'Cross-season validation'],
    results: 'Provided usable yield estimates and feature insights for planning scenarios.',
    gallery: [{ title: 'Yield Study Deployment', src: ghMedia(p7, 'Deployment-3.png') }],
  },
  'defect-detection-manufacturing': {
    introduction: 'Industrial vision project for automated visual defect detection on production lines.',
    problemStatement: 'Manual inspection misses subtle defects and limits line throughput.',
    objectives: ['Detect defects reliably', 'Integrate alerts into operations workflow'],
    methodology: ['Defect annotation pipeline', 'Detection model training', 'Threshold and alert policy tuning'],
    results: 'Improved defect visibility and accelerated quality-control feedback loops.',
    gallery: [{ title: 'Defect Analytics', src: ghMedia(p4, 'results.png') }],
  },
  'document-layout-ai': {
    introduction: 'Document AI pipeline for extracting structured data from scanned and born-digital files.',
    problemStatement: 'Manual document extraction is slow and error-prone in operations teams.',
    objectives: ['Extract key fields and tables', 'Improve extraction confidence and traceability'],
    methodology: ['OCR + layout parsing', 'Post-processing with confidence rules', 'Validation set evaluation'],
    results: 'Reduced extraction turnaround time and improved structured-document consistency.',
    gallery: [{ title: 'Document QA Example', src: ghMedia(p12, 'sample-2.png') }],
  },
  'retail-shelf-monitoring': {
    introduction: 'Retail shelf monitoring using detection and tracking for inventory visibility.',
    problemStatement: 'Stock-outs are frequently detected late in high-traffic stores.',
    objectives: ['Track shelf occupancy trends', 'Trigger low-stock alerts'],
    methodology: ['Object detection with tracking', 'Shelf-level aggregation logic', 'Dashboard event outputs'],
    results: 'Enabled proactive restocking decisions with near real-time shelf insights.',
    gallery: [{ title: 'Monitoring Dashboard Context', src: ghMedia(p13, 'cover.png') }],
  },
  'traffic-density-estimation': {
    introduction: 'Traffic density estimation from road camera streams for congestion analytics.',
    problemStatement: 'Traffic operations teams need continuous density indicators without manual counting.',
    objectives: ['Estimate lane-wise density', 'Provide congestion score outputs'],
    methodology: ['Detection/counting in video streams', 'Lane mapping and aggregation', 'Temporal smoothing'],
    results: 'Produced actionable congestion signals for traffic monitoring use cases.',
    gallery: [{ title: 'Traffic Dataset Samples', src: ghMedia(p5, 'fig2-dataset-samples.png') }],
  },
  'human-pose-safety': {
    introduction: 'Pose-estimation safety monitoring system for workplace compliance checks.',
    problemStatement: 'Unsafe postures and PPE non-compliance are hard to monitor continuously.',
    objectives: ['Detect unsafe pose patterns', 'Support real-time safety alerts'],
    methodology: ['Pose model inference', 'Rule-based safety engine', 'Event logging and review'],
    results: 'Improved safety event visibility for supervised environments.',
    gallery: [{ title: 'Pose Localization Context', src: ghMedia(p10, 'cover.png') }],
  },
  'rag-knowledge-assistant': {
    introduction:
      'Retrieval-augmented knowledge assistant for enterprise document Q&A with source-grounded responses.',
    problemStatement: 'Teams lose time searching disconnected documentation and SOP repositories.',
    objectives: ['Build grounded Q&A workflow', 'Maintain citation-backed answers'],
    methodology: ['Document ingestion and chunking', 'Hybrid retrieval and reranking', 'Prompt + eval iteration'],
    results: 'Delivered faster knowledge retrieval and improved answer reliability.',
    gallery: [{ title: 'VQA/Assistant Context', src: ghMedia(p12, 'sample-3.png') }],
  },
}
